import type { DependencyInfo, RawScanData, ScanSummary, RiskLevel } from '../types/index.js';
import { normalizeScanData, calculateRiskLevel, normalizeSeverity } from '../normalizer/index.js';
...
export async function analyzeDependencies(
  ...
  for (const vuln of rawData.vulnerabilities) {
    if (knownNames.has(vuln.name) || options.ignore.includes(vuln.name)) continue;
...
export function computeSummary(dependencies: DependencyInfo[]): ScanSummary {
  return {
